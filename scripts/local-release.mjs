...
const packages = [
	{ directory: "packages/telemetry", name: "@step-harness/telemetry" },
	{ directory: "packages/providers", name: "@step-harness/providers" },
	{ directory: "packages/tui", name: "@step-harness/pi-tui" },
	{ directory: "packages/agent-core", name: "@step-harness/agent-core" },
	{ directory: "packages/config", name: "@step-harness/config" },
	{ directory: "packages/contracts", name: "@step-harness/contracts" },
	{ directory: "packages/coding-agent", name: "@step-harness/coding-agent" },
];

function printUsage() {
	console.log(`Usage: node scripts/local-release.mjs [options]

Builds and packs internal workspace packages, then installs the tarballs into an
isolated directory outside the repository for local runtime smoke testing. This
is a development check only; StepCode releases standalone binaries.
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `step-${platform}.zip` : `step-${platform}.tar.gz`;
...
function createCliShims(installDirectory) {
	createCommandShim(installDirectory, "step");
}
...
if (rootPackageJson.name !== "stepcode") {
...
for (const pkg of packages) {
	...
	run("npm", ["run", pkg.directory === "packages/providers" ? "build:offline" : "build"], { cwd: pkg.directory });
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `step-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "step.exe" : "step")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "step.cmd" : "step")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "step.cmd" : "step")} --help`);
	}
}
