...
```bash
npm install @step-harness/telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@step-harness/telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@step-harness/telemetry';
...
`@step-harness/telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@step-harness/telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@step-harness/agent-core';
...
- `@step-harness/telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@step-harness/providers` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@step-harness/agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
