...
// Resolve lazily. Step's embedded facade can install its storage bridge after
// this module has been imported; caching getBinDir() at module evaluation time
// would pin downloads to Pi's default ~/.pi/agent/bin directory.
function getToolsDir(agentDir?: string): string {
	const resolvedAgentDir = agentDir?.trim() || (isStepStorageContext() ? resolveStepAgentDir() : undefined);
	return resolvedAgentDir ? join(resolvedAgentDir, "bin") : getBinDir();
}
...
// Check if a command exists in PATH by trying to run it. Exported so grep/find
// fallback backends can probe for `git`, `grep`, `find` without duplicating the
// same spawn-and-swallow-ENOENT dance.
...
export function getToolPath(tool: "fd" | "rg", agentDir?: string): string | null {
	const config = TOOLS[tool];
	if (!config) return null;

	// Check our tools directory first
	const localPath = join(getToolsDir(agentDir), config.binaryName + (platform() === "win32" ? ".exe" : ""));
...
async function downloadTool(tool: "fd" | "rg", agentDir?: string): Promise<string> {
	const config = TOOLS[tool];
	...
	const toolsDir = getToolsDir(agentDir);
...
export interface EnsureToolOptions {
	/** Agent directory used for managed binary storage. Defaults to Pi's agent directory. */
	agentDir?: string;
}
...
export async function ensureTool(
	...
	const existingPath = getToolPath(tool, options?.agentDir);
	if (existingPath) {
		return existingPath;
	}
	...
	try {
		const path = await downloadTool(tool, options?.agentDir);
