...
export class DefaultPackageManager implements PackageManager {
		...
		for (const { pkg, scope } of sources) {
			...
			if (parsed.type === "git") {
				...
				if (!existsSync(installedPath)) {
					const installed = await installMissing();
					if (!installed) continue;
				} else if (resolvedScope === "temporary" && !parsed.pinned) {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// @step-harness/* (and legacy-scope) peers. Stale auto-installed peers can otherwise block updates.
	...
	private getNpmInstallRoot(scope: SourceScope, temporary: boolean): string {
		...
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.configDirName, "npm");
		}
	...
	private getManagedNpmInstallPath(source: NpmSource, scope: SourceScope): string {
		...
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.configDirName, "npm", "node_modules", source.name);
		}
	...
	private getGitInstallRoot(scope: SourceScope): string | undefined {
		...
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.configDirName, "git");
		}
	...
	private getBaseDirForScope(scope: SourceScope): string {
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.configDirName);
		}
		if (scope === "user") {
