...
export class DefaultResourceLoader implements ResourceLoader {
	...
	constructor(options: DefaultResourceLoaderOptions) {
		...
		this.configDirName = options.configDirName?.trim() || CONFIG_DIR_NAME;
		this.settingsManager =
			options.settingsManager ??
			SettingsManager.create(this.cwd, this.agentDir, { configDirName: this.configDirName });
	...
	async reload(options?: ResourceLoaderReloadOptions): Promise<void> {
		...
		await this.settingsManager.reload();
		const resolvedPaths = await this.packageManager.resolve(undefined, { includeSkills: !this.noSkills });
	...
	private async loadCurrentExtensionSet(options: { includeInlineFactories: boolean }): Promise<LoadExtensionsResult> {
		const resolvedPaths = await this.packageManager.resolve(undefined, { includeSkills: false });
		const cliExtensionPaths = await this.packageManager.resolveExtensionSources(this.additionalExtensionPaths, {
			temporary: true,
		});
	...
	private getDefaultSourceInfoForPath(filePath: string): SourceInfo {
		...
		const projectRoots = [
			join(this.cwd, this.configDirName, "skills"),
			join(this.cwd, this.configDirName, "prompts"),
			join(this.cwd, this.configDirName, "themes"),
			join(this.cwd, this.configDirName, "extensions"),
		];
	...
	private loadThemes(
		...
		if (includeDefaults) {
			const defaultDirs = [join(this.agentDir, "themes"), join(this.cwd, this.configDirName, "themes")];
	...
	private discoverSystemPromptFile(): string | undefined {
		const projectPath = join(this.cwd, this.configDirName, "SYSTEM.md");
	...
	private discoverAppendSystemPromptFile(): string | undefined {
		const projectPath = join(this.cwd, this.configDirName, "APPEND_SYSTEM.md");
		if (this.settingsManager.isProjectTrusted() && existsSync(projectPath)) {
			return projectPath;
		}
