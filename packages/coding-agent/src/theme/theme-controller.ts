...
export class InteractiveThemeController {
	...
	private readonly defaultTheme: string | undefined;
	...
	constructor(
		...
		this.activeThemeName = resolveThemeSetting(this.getConfiguredThemeSetting(), this.terminalTheme);
	...
	getThemeSelection(): string | undefined {
		return this.getConfiguredThemeSetting() ?? this.activeThemeName;
	}
	...
	private getConfiguredThemeSetting(): string | undefined {
		return this.currentThemeSetting ?? this.getSettingsManager().getThemeSetting() ?? this.defaultTheme;
	}
	...
	private applyTerminalTheme(terminalTheme: TerminalTheme): void {
		...
		const autoTheme = parseAutoThemeSetting(this.getConfiguredThemeSetting());
		if (!autoTheme) {
			this.setAutoSync(false);
			return;
		}
