...
const ThemeJsonSchema = Type.Object({
	...
	colors: Type.Object({
		...
		codeInlineBg: Type.Optional(ColorValueSchema),
		...
		mdTableHeader: Type.Optional(ColorValueSchema),
...
type OptionalThemeColor = "thinkingMax" | "searchMatchText" | "mdTableHeader";
type OptionalThemeBg = "scrollbarThumb" | "searchMatchBg" | "codeInlineBg";
...
function withThemeColorFallbacks(colors: ThemeJson["colors"]): ThemeJson["colors"] & {
	...
	return {
		...
		codeInlineBg: colors.codeInlineBg ?? colors.customMessageBg,
		mdTableHeader: colors.mdTableHeader ?? colors.mdHeading,
	};
}
...
export class Theme {
	...
	constructor(
		...
		const colors = {
			...
			mdTableHeader: fgColors.mdTableHeader ?? fgColors.mdHeading,
		};
		...
		const backgrounds = {
			...
			codeInlineBg: bgColors.codeInlineBg ?? bgColors.customMessageBg,
		};
	...
	// Attribute helpers emit raw SGR instead of delegating to Chalk, which
	// disables attributes for captured/non-TTY output — the same reason the
	// Step welcome title bolds explicitly. Raw sequences keep bold visible (and
	// testable) in piped output, exported transcripts, and unit tests alike.
	...
	if (!BUILTIN_THEMES) {
		...
		for (const name of ["sage", "step-blue", "step-violet", "step-violet-light"] as const) {
			...
			if (fs.existsSync(themePath)) {
				themes[name] = JSON.parse(stripBom(fs.readFileSync(themePath, "utf-8"))) as ThemeJson;
			}
		}
...
function getCustomThemeInfos(): ThemeInfo[] {
	const customThemesDir = getThemeCustomThemesDir();
	const result: ThemeInfo[] = [];
...
function parseThemeJson(label: string, json: unknown): ThemeJson {
	if (!validateThemeJson.Check(json)) {
		...
		if (missingColors.size > 0) {
			...
			errorMessage +=
				"\nSee the built-in themes (dark.json, light.json, sage.json, step-blue.json, step-violet.json, step-violet-light.json) for reference values.";
		}
...
const THEME_KEY_NEW = Symbol.for("@step-harness/coding-agent:theme");
// A1: legacy scope keys are kept so a theme set by an older-scoped build is still found,
// and one we set stays visible to it (only-add, never remove — §7.8.5).
...
export const theme: Theme = new Proxy({} as Theme, {
	get(_target, prop) {
		const store = globalThis as Record<symbol, Theme>;
		const t = store[THEME_KEY_NEW] ?? store[THEME_KEY] ?? store[THEME_KEY_OLD];
...
function setGlobalTheme(t: Theme): void {
	const store = globalThis as Record<symbol, Theme>;
	store[THEME_KEY_NEW] = t;
	store[THEME_KEY] = t;
	store[THEME_KEY_OLD] = t;
}
...
/**
 * Optional instance boundary supplied by a product wrapper. Pi's public theme
 * helpers retain their normal defaults when this is unset; Step sets it before
 * mounting a TUI so custom themes cannot be read from the host Pi directory.
 */
let themeStorageAgentDir: string | undefined;

export function setThemeStorageDir(agentDir?: string): void {
	themeStorageAgentDir = agentDir?.trim() ? path.resolve(agentDir) : undefined;
}

function getThemeCustomThemesDir(): string {
	return themeStorageAgentDir ? path.join(themeStorageAgentDir, "themes") : getCustomThemesDir();
}
...
export function getResolvedThemeColors(themeName?: string): Record<string, string> {
	...
	for (const [key, value] of Object.entries(resolved)) {
		if (typeof value === "number") {
			...
			cssColors[key] = key.endsWith("Bg") || key === "scrollbarThumb" ? "transparent" : defaultText;
		} else {
			cssColors[key] = value;
		}
	}
...
export function isLightTheme(themeName?: string): boolean {
	return themeName === "light" || themeName === "step-violet-light";
}
...
export function getMarkdownTheme(activeTheme: Theme = theme): MarkdownTheme {
	return {
		heading: (text: string) => activeTheme.fg("mdHeading", text),
		// The literal `#` markers are syntax noise, not content — dim them so the
		// colored heading text carries the hierarchy (Codex / reference style).
		headingPrefix: (text: string) => activeTheme.fg("muted", text),
		// Table headers get their own hue so the head/body boundary reads at a
		// glance, matching the two-tone emphasis of the reference rendering.
		tableHeader: (text: string) => activeTheme.bold(activeTheme.fg("mdTableHeader", text)),
		link: (text: string) => activeTheme.fg("mdLink", text),
		linkUrl: (text: string) => activeTheme.fg("mdLinkUrl", text),
		code: (text: string) =>
			activeTheme.getBgAnsi("codeInlineBg") === "\x1b[49m"
				? activeTheme.fg("mdCode", text)
				: activeTheme.bg("codeInlineBg", activeTheme.fg("mdCode", ` ${text} `)),
		codeBlock: (text: string) => activeTheme.fg("mdCodeBlock", text),
		// The opening fence doubles as the language label; tint the tag with the
		// accent so the block's language is scannable at a glance.
		codeBlockBorder: (text: string) => {
			const match = text.match(/^(```+)(.*)$/u);
			if (!match || match[2] === undefined || match[2] === "") return activeTheme.fg("mdCodeBlockBorder", text);
			return `${activeTheme.fg("mdCodeBlockBorder", match[1]!)}${activeTheme.fg("mdCode", match[2])}`;
		},
		...
		highlightCode: (code: string, lang?: string): string[] => {
			...
			const opts = {
				language: validLang,
				ignoreIllegals: true,
				theme: getCliHighlightTheme(activeTheme),
			};
			try {
				return highlight(code, opts).split("\n");
			} catch {
...
export function getSelectListTheme(): SelectListTheme {
	return {
		// 选中态靠 → 前缀 + 加粗识别，不再占品牌紫（紫只锚在工具行与门面）
...
export function getSettingsListTheme(): SettingsListTheme {
	return {
		label: (text: string, selected: boolean) => (selected ? theme.bold(theme.fg("text", text)) : text),
		value: (text: string, selected: boolean) =>
			selected ? theme.bold(theme.fg("text", text)) : theme.fg("muted", text),
