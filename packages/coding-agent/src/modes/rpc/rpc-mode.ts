...
export async function runRpcMode(runtimeHost: AgentSessionRuntimeHost): Promise<never> {
	...
	const rebindSession = async (): Promise<void> => {
		...
		// Bind output listeners before extension session_start handlers run. This
		// keeps events emitted during replacement observable to RPC clients.
		unsubscribe?.();
	...
	const registerSignalHandlers = (): void => {
		...
		for (const signal of signals) {
			const handler = () => {
				...
				void shutdown(signal === "SIGHUP" ? 129 : signal === "SIGINT" ? 130 : 143, signal);
			};
	...
	async function shutdown(exitCode = 0, signal?: NodeJS.Signals): Promise<never> {
		...
		shuttingDown = true;
		// Sweep any detached background children on every shutdown path (SIGINT/SIGTERM/
		// SIGHUP and normal RPC shutdown); the signal handlers also sweep eagerly.
		killTrackedDetachedChildren();
