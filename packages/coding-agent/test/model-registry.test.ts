...
import { getApiProvider, getSupportedThinkingLevels } from "@step-harness/providers/compat";
...
describe("ModelRegistry", () => {
	...
	describe("baseUrl override (no custom models)", () => {
		...
		test("overriding baseUrl changes URL on all built-in models", async () => {
			...
			// All models should have the new baseUrl. The trailing `/v1` is dropped
			// because the Anthropic adapter appends the full `/v1/messages` path.
		...
		test("can mix baseUrl override and models merge", async () => {
			...
			expect(openaiModels.some((m) => m.id === "custom-openai-model")).toBe(true);
		});
	...
	describe("custom models merge behavior", () => {
		test("built-in provider custom models inherit api and baseUrl without explicit fields", async () => {
			...
			const model = registry.find("openai", "fake-provider/fake-model");
		...
		test("modelOverrides still apply when provider also defines models", async () => {
			writeRawModelsJson({
				openai: {
					...
					modelOverrides: {
						"gpt-5": {
							name: "Overridden Built-in GPT-5",
						},
					},
				},
			});
			...
			expect(models.some((m) => m.id === "custom/openai-model")).toBe(true);
			expect(models.some((m) => m.id === "gpt-5" && m.name === "Overridden Built-in GPT-5")).toBe(true);
		});
	...
	describe("modelOverrides (per-model customization)", () => {
		test("model override applies to a single built-in model", async () => {
			...
			const sonnet = models.find((m) => m.id === "gpt-5");
			expect(sonnet?.name).toBe("Custom Sonnet Name");

			// Other models should be unchanged
			const opus = models.find((m) => m.id === "gpt-5-mini");
		...
		test("supportsFinishReason can be configured at provider and model levels", async () => {
			...
			const registry = await createModelRegistry(authStorage, modelsJsonPath);
			const models = getModelsForProvider(registry, "openai");
			const sonnet = models.find((model) => model.id === "gpt-5");
			const opus = models.find((model) => model.id === "gpt-5-mini");
		...
		test("refresh() picks up model override changes", async () => {
			...
			const registry = await createModelRegistry(authStorage, modelsJsonPath);
			expect(getModelsForProvider(registry, "openai").find((m) => m.id === "gpt-5")?.name).toBe("First Name");
			...
			expect(getModelsForProvider(registry, "openai").find((m) => m.id === "gpt-5")?.name).toBe("Second Name");
		});
		...
		test("removing model override restores built-in values", async () => {
			...
			const registry = await createModelRegistry(authStorage, modelsJsonPath);
			const customName = getModelsForProvider(registry, "openai").find((m) => m.id === "gpt-5")?.name;
			...
			const restoredName = getModelsForProvider(registry, "openai").find((m) => m.id === "gpt-5")?.name;
