...
describe("parseArgs", () => {
	...
	describe("--resume flag", () => {
		...
		test("--resume with an id resolves via --session instead of opening the selector", () => {
			const result = parseArgs(["--resume", "9b8fe41b-40f1-4f10-a869-1d2a6128a52e"]);
			expect(result.resume).toBeUndefined();
			expect(result.session).toBe("9b8fe41b-40f1-4f10-a869-1d2a6128a52e");
		});

		test("-r with an id resolves via --session", () => {
			const result = parseArgs(["-r", "abc123"]);
			expect(result.resume).toBeUndefined();
			expect(result.session).toBe("abc123");
		});

		test("--resume followed by a flag still opens the selector", () => {
			const result = parseArgs(["--resume", "--print"]);
			expect(result.resume).toBe(true);
	...
	describe("Step tool approval flags", () => {
		test("parses the approval mode and non-interactive fallback", () => {
			expect(parseArgs(["--approval-mode", "auto", "--non-interactive-approval", "allow"])).toMatchObject({
				approvalMode: "auto",
		...
		test("reports invalid approval values without treating them as messages", () => {
	...
	describe("messages and file args", () => {
		...
		// User feedback: `stepcode -p --mode json "@empty.md 这个文件有什么"` treated the whole
		// quoted arg as one filename. A single @-arg containing whitespace must split into a
		// file token plus trailing message text.
		test("splits an @-arg containing whitespace into a file token and a message", () => {
			const result = parseArgs(["@empty.md 这个文件有什么"]);
			expect(result.fileArgs).toEqual(["empty.md"]);
			expect(result.messages).toEqual(["这个文件有什么"]);
		});

		test("reproduces the reported argv: -p --mode json '@file text'", () => {
			const result = parseArgs(["-p", "--mode", "json", "@empty.md 这个文件有什么"]);
			expect(result.print).toBe(true);
			expect(result.mode).toBe("json");
			expect(result.fileArgs).toEqual(["empty.md"]);
			expect(result.messages).toEqual(["这个文件有什么"]);
		...
		test('honors @"quoted path" so spaces in a filename are preserved', () => {
			const result = parseArgs(['@"my notes.md" summarize this']);
			expect(result.fileArgs).toEqual(["my notes.md"]);
			expect(result.messages).toEqual(["summarize this"]);
		});

		test("does not treat '=' or apostrophe as a path boundary", () => {
			const result = parseArgs(["@foo=bar.txt tell me"]);
			expect(result.fileArgs).toEqual(["foo=bar.txt"]);
			expect(result.messages).toEqual(["tell me"]);
		});

		test("keeps a bare @ as message text instead of an empty file arg", () => {
			const result = parseArgs(["@"]);
			expect(result.fileArgs).toEqual([]);
			expect(result.messages).toEqual(["@"]);
		});

		test("a whitespace-free @file yields no spurious empty message", () => {
			const result = parseArgs(["@file.md"]);
			expect(result.fileArgs).toEqual(["file.md"]);
			expect(result.messages).toEqual([]);
		});

		test('tolerates an unclosed @"quote by treating the rest as the path', () => {
			const result = parseArgs(['@"partial']);
			expect(result.fileArgs).toEqual(["partial"]);
			expect(result.messages).toEqual([]);
		});

		test("splits an @-arg with whitespace after the -- delimiter", () => {
			const result = parseArgs(["--", "@empty.md read it"]);
			expect(result.fileArgs).toEqual(["empty.md"]);
			expect(result.messages).toEqual(["read it"]);
		});
	...
	// User feedback: `step --session-dir --version` consumed --version as the
	// directory name, created a "--version" dir and never printed the version.
	// Every value-taking option must reject a missing or flag-like value instead
	// of swallowing the following option.
	describe("value-taking options reject a missing or flag-like value", () => {
		test("--session-dir does not consume a following --version", () => {
			const result = parseArgs(["--session-dir", "--version"]);
			expect(result.sessionDir).toBeUndefined();
			expect(result.version).toBe(true);
			expect(result.diagnostics).toEqual([{ type: "error", message: "--session-dir requires a value" }]);
		});
		...
		test.each(VALUE_FLAGS)("%s reports a missing value when it is the last argument", (flag, canonical) => {
			const result = parseArgs([flag]);
			expect(result.diagnostics).toEqual([{ type: "error", message: `${canonical} requires a value` }]);
		});

		test.each(VALUE_FLAGS)("%s does not swallow a following flag", (flag, canonical) => {
			const result = parseArgs([flag, "--verbose"]);
			expect(result.verbose).toBe(true);
			expect(result.diagnostics).toEqual([{ type: "error", message: `${canonical} requires a value` }]);
		});

		test("normal values are still accepted for every value-taking option", () => {
			const result = parseArgs([
				"--provider",
				"openai",
				"--model",
			...
			expect(result.tools).toEqual(["read", "bash"]);
			expect(result.thinking).toBe("high");
			expect(result.themes).toEqual(["/tmp/theme.json"]);
			expect(result.diagnostics).toEqual([]);
		});

		test("an invalid --thinking value still warns rather than erroring", () => {
			const result = parseArgs(["--thinking", "bogus"]);
			expect(result.thinking).toBeUndefined();
			expect(result.diagnostics[0].type).toBe("warning");
		});

		// An option token never contains whitespace, so dash-leading free text is a
		// value, not a flag: a prompt opening with a markdown bullet or YAML front
		// matter must still be accepted.
		test("a dash-leading value containing whitespace is accepted as a value", () => {
			const result = parseArgs(["--system-prompt", "- Always use tabs", "-p", "hi"]);
			expect(result.systemPrompt).toBe("- Always use tabs");
			expect(result.print).toBe(true);
			expect(result.messages).toEqual(["hi"]);
			expect(result.diagnostics).toEqual([]);
		});

		test("a prompt beginning with YAML front matter is accepted", () => {
			const prompt = "---\nname: x\n---\nbe terse";
			const result = parseArgs(["--append-system-prompt", prompt, "hi"]);
			expect(result.appendSystemPrompt).toEqual([prompt]);
			expect(result.messages).toEqual(["hi"]);
			expect(result.diagnostics).toEqual([]);
		});

		test("a whitespace-free flag-like token is still rejected", () => {
			const result = parseArgs(["--system-prompt", "--verbose"]);
			expect(result.systemPrompt).toBeUndefined();
			expect(result.verbose).toBe(true);
			expect(result.diagnostics).toEqual([{ type: "error", message: "--system-prompt requires a value" }]);
		});
	});
});
