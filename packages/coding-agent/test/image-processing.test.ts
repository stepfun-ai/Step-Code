...
import { imageFileToContent } from "../src/utils/image-process.ts";
...
describe("resizeImage", () => {
	...
	it("re-encodes at original dimensions when exceeding the byte limit", async () => {
		...
		// Claude Code parity: a byte-only overage re-encodes at the original
		// dimensions instead of shrinking the image, and never returns null.
		expect(result).not.toBeNull();
	...
	it("returns a best-effort JPEG instead of dropping the image when maxBytes is unreachable", async () => {
		// Claude Code parity: an unreachable byte budget must not omit the
		// image — the last-resort JPEG (quality 20, up-to-1000px width) is
		// returned even though it still exceeds maxBytes.
		const result = await resizeImage(imageBytes(LARGE_PNG_200x200), "image/png", {
			maxWidth: 2000,
			maxHeight: 2000,
			maxBytes: 1,
...
describe("imageFileToContent", () => {
	let dir: string;

	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "image-file-to-content-"));
	});
	...
	it("reads a PNG file into an inline image attachment", async () => {
		const filePath = join(dir, "pic.png");
		writeFileSync(filePath, imageBytes(TINY_PNG));

		const content = await imageFileToContent(filePath, { autoResizeImages: false });
	...
	it("returns null for a non-image file", async () => {
		...
		writeFileSync(filePath, "just text, not an image");

		expect(await imageFileToContent(filePath, { autoResizeImages: false })).toBeNull();
	});
});
