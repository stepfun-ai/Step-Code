...
import { initTheme, type TerminalTheme, theme } from "../src/theme/theme.ts";
import { InteractiveThemeController } from "../src/theme/theme-controller.ts";
...
describe("InteractiveThemeController", () => {
	...
	it("uses the injected default theme without reading product environment variables", () => {
		vi.stubEnv("STEPCODE_DEFAULT_THEME", "light");
		const { ui } = createUi();
		const manager = SettingsManager.inMemory({});
		const controller = createController(ui, () => manager, undefined, "step-violet");

		expect(theme.name).toBe("step-violet");
		expect(controller.getThemeSelection()).toBe("step-violet");
	});
