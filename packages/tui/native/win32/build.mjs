...
Environment:
  STEP_TUI_WIN32_TOOLCHAIN=msvc|mingw
...
const requestedToolchain = process.env.STEP_TUI_WIN32_TOOLCHAIN;
if (requestedToolchain && requestedToolchain !== "msvc" && requestedToolchain !== "mingw") {
	throw new Error("STEP_TUI_WIN32_TOOLCHAIN must be either 'msvc' or 'mingw'");
}
...
if (toolchain === "msvc") {
	if (!vsDevCmd || !hasMsvc) {
		throw new Error("Microsoft C++ Build Tools not found. Install the Visual Studio C++ workload or set STEP_TUI_WIN32_TOOLCHAIN=mingw.");
	}
