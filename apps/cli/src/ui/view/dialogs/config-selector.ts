...
function getGroupLabel(metadata: PathMetadata, agentDir: string, configDirName: string): string {
	...
	if (metadata.source === "auto") {
		...
		return metadata.scope === "user" ? `User (${formatBaseDir(agentDir)})` : `Project (${configDirName}/)`;
	}
	return metadata.scope === "user" ? "User settings" : "Project settings";
}

function buildGroups(resolved: ResolvedPaths, agentDir: string, configDirName: string): ResourceGroup[] {
		...
		for (const res of resources) {
			...
			if (!groupMap.has(groupKey)) {
				groupMap.set(groupKey, {
					key: groupKey,
					label: getGroupLabel(metadata, agentDir, configDirName),
...
class ConfigSelectorHeader implements Component {
	...
	constructor(writeScope: ConfigWriteScope, projectModeAvailable: boolean, configDirName: string) {
	...
	render(width: number): string[] {
		...
		const scopeHint =
			this.writeScope === "project"
				? theme.fg("muted", `${this.configDirName}/config.toml · inherited global resources are dimmed`)
				: theme.fg("muted", `~/${this.configDirName}/config.toml`);
...
export class ConfigSelectorComponent extends Container implements Focusable {
	...
	constructor(
		...
		this.writeScope = writeScope;
		const groupsByScope = {
			global: buildGroups(resolvedPaths.global, agentDir, configDirName),
			project: buildGroups(resolvedPaths.project, agentDir, configDirName),
		};
		...
		this.addChild(new Spacer(1));
		this.header = new ConfigSelectorHeader(this.writeScope, projectModeAvailable, configDirName);
