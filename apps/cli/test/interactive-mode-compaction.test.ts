import type { Usage } from "@step-harness/providers";
...
describe("InteractiveMode compaction events", () => {
	test("uses the cache miss notice setting for compaction and branch summary costs", () => {
		...
		const enabled = {
			...
			settingsManager: { getShowCacheMissNotices: () => true, getStatusTips: () => false },
		...
		const disabled = {
			chatContainer: new Container(),
			settingsManager: { getShowCacheMissNotices: () => false, getStatusTips: () => false },
		};
	...
	test("renders retained entries and appends the latest summary cost at the bottom", async () => {
		...
		const fakeThis = {
			...
			sessionManager: { buildContextEntries: vi.fn().mockReturnValue([latestCompaction, previousCompaction]), getBranch: () => [], getSessionId: () => "session-test" },
			...
			settingsManager: { getShowTerminalProgress: () => false, getStatusTips: () => false },
	...
	test("updates the working state when the same agent run resumes after compaction", async () => {
		const fakeThis = {
			...
			clearStatusIndicator: vi.fn(),
			settingsManager: { getShowTerminalProgress: () => true, getStatusTips: () => false },
		...
		expect(fakeThis.redraw.requestRender).toHaveBeenCalledTimes(2);
	});
