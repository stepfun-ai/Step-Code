...
import { initTheme, type Theme, theme } from "../../../../../packages/coding-agent/src/theme/theme.ts";
...
function createReloadCommandContext(overrides: ReloadCommandContextOverrides = {}): ReloadCommandContext {
	...
	return {
		...
		redraw: { requestRender: () => {}, forceRender: () => {}, renderNow: () => {} },
		editor,
	...
	it("renders replacement session state before session_start handlers can notify", async () => {
		...
		try {
			const context: RebindContext = {
				...
				updateTerminalTitle: () => {},
				// renderBeforeBind commits the startup frame before binding extensions.
				redraw: { renderNow: () => {} },
			};
