...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@step-harness/pi-tui";
import { beforeAll, describe, expect, test, vi } from "vitest";
import { type Component, Container, type Focusable, type TUI } from "../../../packages/tui/src/tui.ts";
...
import type { AutocompleteProviderFactory } from "../../../packages/coding-agent/src/core/extensions/types.ts";
import type { SourceInfo } from "../../../packages/coding-agent/src/core/source-info.ts";
import type { AuthSelectorProvider } from "../src/ui/view/dialogs/oauth-selector.ts";
...
describe("InteractiveMode.showManagedToolStatus", () => {
	...
	test("renders tool updates as one contiguous group", () => {
		...
		// Warnings (download failure / offline / unsupported) are suppressed: a
		// missing search tool is non-fatal and falls back to git/POSIX silently.
...
describe("InteractiveMode.setToolsExpanded", () => {
	test("applies expansion state to the active header and chat entries", () => {
		...
		const widgetAbove = { setExpanded: vi.fn() };
		const widgetBelow = { setExpanded: vi.fn() };
		const fakeThis: any = {
			...
			extensionWidgetsAbove: new Map([["above", widgetAbove]]),
			extensionWidgetsBelow: new Map([["below", widgetBelow]]),
			ui: { requestRender: vi.fn() },
			redraw: { requestRender: vi.fn(), forceRender: vi.fn(), renderNow: vi.fn() },
			showStatus: vi.fn(),
		};
		...
		expect(widgetAbove.setExpanded).toHaveBeenCalledWith(true);
		expect(widgetBelow.setExpanded).toHaveBeenCalledWith(true);
...
describe("InteractiveMode.showExtensionCustom", () => {
	...
	test("overlay custom UI reclaims input after non-overlay custom UI closes", async () => {
		...
		const fakeThis = {
			...
			keybindings: {},
			ui,
			redraw: { requestRender: () => ui.requestRender(), forceRender: () => ui.requestRender(true), renderNow: () => ui.renderNow() },
...
describe("InteractiveMode.createBaseAutocompleteProvider", () => {
	test("offers the same thinking-level completions for /thinking and /effort", async () => {
		...
		const getAvailableThinkingLevels = vi.fn(() => ["off", "low", "high"]);
		...
		const getCompletions = (line: string) =>
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createShowLoadedResourcesThis(options: {
		...
		const fakeThis: any = {
			options: {
				...
				tuiStyle: options.tuiStyle ?? "native",
			},
	...
	test("hides ordinary resource sections in Step presentation", () => {
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			tuiStyle: "step",
	...
	test("keeps resource diagnostics visible in Step presentation", () => {
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			tuiStyle: "step",
			skills: [{ filePath: "/tmp/skill/SKILL.md", name: "commit" }],
...
"[Extensions]
  ...
  path
	...
	test("treats only projected conversation messages as a started session", () => {
		const hasConversationMessages = (InteractiveMode as any).prototype.hasConversationMessages as (
			entries: unknown[],
		) => boolean;
		...
		expect(hasConversationMessages.call({}, metadataEntries)).toBe(false);
		...
		expect(hasConversationMessages.call({}, [...metadataEntries, userEntry])).toBe(true);
	});
